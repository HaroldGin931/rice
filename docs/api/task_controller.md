...
新任务奖励由所属社区的可用稻米承担。发布时冻结；结果被认可时发给承作人并生成
`task_reward` 流水；任命前取消则退回原出资账户。`funding_node_id` 由服务器固定，
未发布的旧草稿在发布时也采用社区账户；已发布的历史记录保留 null，继续原发布者个人资金归属。
个人余额不因管理员角色自动转移。每笔冻结与其退款或结算有唯一凭证，详见[社区钱包](wallet_and_inbox.md)。
...
撤销管理员角色后，新社区资金任务的管理权和其他申请人的私有联系方式立即停止返回；
历史个人出资任务仍保留原发布者的既有管理权限。公共响应不因此公开申请信息。
...
组织方可审核当前待审提交。成功后在同一事务中进入 `completed`、释放原出资账户冻结余额、
