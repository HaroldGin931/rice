...
2026-09-21：公开社区目录与独立入会申请。节点的 `user_id` 是创始管理员；其他管理员由
`node_memberships.role=admin` 授权，普通成员为 `member`。成员身份来自
...
| `POST /api/nodes/:node_id/applications/:application_id/approve` | 社区管理员通过申请并建立成员身份 |
| `POST /api/nodes/:node_id/applications/:application_id/reject` | 社区管理员拒绝；`review_reason` 可选、最长 512 |
| `PATCH /api/nodes/:node_id/members/:user_id` | 创始管理员设定现有成员 `role: admin\|member`；不得修改创始管理员或直接授权非成员 |
...
`can_manage_members` 仅对创始管理员为 `true`，普通管理员可审批入会、共同管理业务，但不能增删管理员。
角色撤销后，后续请求立即按普通成员检查权限。个人稻米账户不因管理员角色而改变。
...
不把旧名单当作社区身份来源。创建节点、移出成员和创始管理员交接不在当前用户流程中。
