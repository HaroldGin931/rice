...
| 状态码 | `error` | `errors.detail` | 什么时候 |
| --- | --- | --- | --- |
| `401` | `InvalidCredentials` | 账号或密码错误 | 账号不存在**或**密码错 |
| `403` | `AccountDisabled` | 该账号已被禁用 | 被后台停用 |
| `422` | `InvalidLoginRequest` | 请输入账号和密码。 | 缺少或传入非字符串账号、密码 |
| `503` | `LoginUnavailable` | 登录服务暂时不可用，请稍后重试。 | PDS 网络或服务故障 |

前端按 `error` 区分错误，`errors.detail` 保留供现有客户端显示。登录请求的 `401`
表示凭据被拒绝，不能当作已有登录状态过期；服务故障也不能提示密码错误。
